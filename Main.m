%%%% This code generates a Customized G-code for Solid geometries like beam,cube etc. %%%%
%%%% Customized tool-path is generated by defining a continuous nodal path with uniform flow rate %%
%%%% Written By Dhanashri Desai (3rd year,MTech(R),ME,IISc) during 9 Dec'22 to 15 Dec'22 %%%%

clear; clc; close all; % Housekeeping commands
l=26; b=26; h=min(l,b); % size parameters of solid model
hLayer=2; % layer height
bLayer=3;% layer width (2 for solid c/s and >2 for hollow c/s)
E0=0;  %E0=5.87; 
dLdE=60; % uniform rate at which motor feed increases
nL=5; %round(h/hLayer); % no. of layers to be printed
nElem=round(b/bLayer); nnodes=2*nElem;
x0=60; y0=140;
...
for i=1:length(nodeID1)
    nx(nodeID1(i))=x0+(bLayer/2)+(i-1)*bLayer; ny(nodeID1(i))=y0;
    nx(nodeID2(i))=x0+(bLayer/2)+(i-1)*bLayer; ny(nodeID2(i))=abs(l-y0);
end
nx=nx'; ny=ny';
...
NodalPath=NodalPath';
disp([NodalPath nx ny]);

%% generate uniform flow rate data according to nodal path
nExtr=length(NodalPath)-1;
for i=1:nL
        for j=1:nExtr
            if i==1 && j==1
                E((i-1)*nExtr+j)=E0+l/dLdE;
            elseif mod(j,2)==0
                E((i-1)*nExtr+j)=E((i-1)*nExtr+j-1)+bLayer/dLdE;
            else
...
if l==b
    fileID = fopen('CubeGcode.gcode','wt'); 
else
    fileID = fopen('BeamGcode.gcode','wt'); 
end
...
if l==b
    winopen('CubeGcode.gcode'); 
else
    winopen('BeamGcode.gcode'); 
end
